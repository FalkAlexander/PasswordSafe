...
class BrowsingPanel(Adw.Bin):
    ...
    def __init__(self, unlocked_database):
        ...
        self.list_model = Gtk.FilterListModel.new(flatten_model, self._filter)

        self.selection_model = SingleSelection(self.list_model)
    ...
    def set_search(self, query: str | None) -> None:
        ...
        if query is None:
            self.list_model.props.filter = self._filter
            self._set_stack_page()
            self._query = ""
            return

        self.list_model.props.filter = self._search_filter
    ...
    def _set_stack_page(self):
        if not self.list_model.get_n_items():
