...
class UnlockedDatabase:
    ...
    def register_special_keys(self):
        self.window.connect("key-press-event", self.on_special_key_pressed)
        self.window.connect("key-release-event", self.on_special_key_released)
    ...
    def on_special_key_pressed(self, window, eventkey):
        ...
        if self.window.container.page_num(self.parent_widget) == self.window.container.get_current_page():
            ...
            if self.database_locked is False and self.selection_ui.selection_mode_active is False and self.stack.get_visible_child() is not self.stack.get_child_by_name("search") and scrolled_page.edit_page is True:
    ...
    def on_special_key_released(self, window, eventkey):
        ...
        if self.window.container.page_num(self.parent_widget) == self.window.container.get_current_page():
            scrolled_page = self.stack.get_child_by_name(group_uuid)
            if self.database_locked is False and self.selection_ui.selection_mode_active is False and self.database_manager.check_is_group(self.database_manager.get_group_uuid_from_group_object(self.current_group)) and scrolled_page.edit_page is False:
